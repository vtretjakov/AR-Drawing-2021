...
class ViewController: UIViewController {
    ...
    /// Arrays of object placed(Массивы найденных размещенных объектов)
    var objectPlaced = [SCNNode]()
    
    /// Array of found planes(Массивы найденных плоскостей)
    var planeNodes = [SCNNode]()
    
    
    /// The node for the object currently selected by the user (Узел для объекта, выбранного пользователем в данный момент)
    var selectedNode: SCNNode? /// ? - т.к может быть не выбрано
    
    /// Visualize planes
    var arePlanesHidden = true {
        didSet {
            planeNodes.forEach {$0.isHidden = !arePlanesHidden} /// for planeNode in planeNodes { planeNode.isHidden = arePlanedHidden }  - идентичное выражение, $0 - planeNode in planeNode - аналогично.
        }
    }
    
    // MARK: - Methods
    /// Add a node user's touch location represented by point
    /// - Parameters:
    /// - node: the node to be added
    /// - point: point at which user has touches the screen (точка, в которой пользователь коснулся экрана)
    ...
    func addNode(_ node: SCNNode, to parentNode: SCNNode) {
        // Check that the object is not too close to the previous one (проверка насколько объект близок к предыдущему)
...
extension ViewController: ARSCNViewDelegate {
    func createFloor(with size: CGSize, opacity: CGFloat = 0.25) -> SCNNode {
        // Get estimated plane size (получить предположительный размер)
    ...
    func nodeAdded(_ node: SCNNode, for anchor: ARPlaneAnchor) {
        ...
        // Add plane node to the list plane nodes
    ...
    func updateFloor(for node: SCNNode, anchor: ARPlaneAnchor) {
        guard let planeNode = node.childNodes.first, let plane = planeNode.geometry as? SCNPlane else {
            print(#line, #function, "Can't find SCNPlane at node \(node)") // Не удается найти SCNPlane в узле
            return
        }
